...
import {
    ...
    Settings, Gamepad2, Edit3, Monitor, ShoppingCart, Play, LogOut, X, Globe, Volume2, VolumeX, Paintbrush
} from 'lucide-react';
    ...
    return (
        <div className={`absolute inset-0 z-50 flex flex-col items-center backdrop-blur-md overflow-hidden ${weedMode ? 'bg-green-950/95' : 'bg-black/90'}`}>
            ...
            <div className="fixed top-2 right-2 z-[100] flex flex-col gap-2 items-end">
                ...
                <button
                    onClick={toggleMute}
                    className={`p-2 rounded-xl border transition-all shadow-lg flex items-center justify-center ${!isMuted
                ...
                <div className={`w-full border rounded-xl p-3 backdrop-blur-sm flex flex-col items-center shrink-0 relative z-10 transition-all ${weedMode ? 'bg-green-900/20 border-green-800/50' : 'bg-slate-900/40 border-slate-800/50'}`}>
                    ...
                    <div className="w-full mt-1">
                        ...
                        <div className="flex gap-2 w-full overflow-x-auto custom-scrollbar pb-2 px-1 snap-x">
                            ...
                            {/* PIXEL ART EDITOR Button */}
                            <button
                                onClick={() => setShowPixelEditor(true)}
                                className="flex-shrink-0 flex flex-col items-center justify-center w-10 h-10 rounded-lg border border-orange-500/30 bg-orange-900/20 hover:bg-orange-900/40 relative overflow-hidden"
                            >
                                <Paintbrush size={12} className="text-orange-400" />
                            </button>
                            ...
                            {safeSkins.map((skin: any, i: number) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`flex-shrink-0 w-10 h-10 p-1 rounded-lg border transition-all relative snap-center ${isLocked ? 'border-red-900/30 bg-red-950/20' :
            ...
            {showPixelEditor && (
                <PixelArtEditor
                    ...
                    onSave={(pixels, name, color) => {
                        // Create custom character
                        const customSkin: CharacterSkin = {
                            id: `custom_${Date.now()}`,
                            name: name,
                            color: color,
                            lore: {
                                PT: "Personagem customizado criado por você!",
                                EN: "Custom character created by you!",
                                IT: "Personaggio personalizzato creato da te!"
                            },
                            pixels: pixels
                        };

                        // Select the new character immediately
                        setGameState((prev: any) => ({ ...prev, selectedSkin: customSkin }));
